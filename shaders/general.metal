...
/*
    ...
    for more info.
*/
fragment float4 fragmentMain(VertexPayload frag [[stage_in]], texturecube<float> skyBox [[texture(1)]],
                             texture2d<float> shadowPass [[texture(2)]], constant AmbientLight &al [[buffer(3)]],
                             constant DirectionalLight &dl [[buffer(4)]], constant PointLight &pl [[buffer(5)]],
    ...
    const auto colorSample      = float4(frag.color,1.0f);
    const auto specularSample   = float4(0.0f);
    const auto roughnessSample   = float4(0.0f);

    const auto ambient_light    = calcAmbientLight(al, colorSample);

    const auto normal           = normalize(frag.normal);
