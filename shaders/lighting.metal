...
kernel void mainFunction(texture2d<float> geometryPass [[texture(0)]], texture2d<float> normalPass [[texture(1)]],
                         texture2d<float> colorPass [[texture(2)]], texture2d<float> specularPass [[texture(3)]],
                         texture2d<float> roughnessPass [[texture(4)]],
                         texture2d<float, access::write> outputTex [[texture(5)]],
                         constant AmbientLight &al [[buffer(6)]], constant DirectionalLight &dl [[buffer(7)]],
                         constant PointLight &pl [[buffer(8)]], constant Camera &camera [[buffer(9)]],
  ...
  const auto roughnessSample  = roughnessPass.sample(textureSampler, uv);
  ...
  const auto point_light       = calcSpecularGGX(pl.position, camera.position, fragWorldPosition.xyz, normal,
                                                pl.colour.rgb * pl.strength, // light color/intensity
                                                colorSample.rgb, // albedo from texture
                                                roughnessSample.r, specularSample.r);
